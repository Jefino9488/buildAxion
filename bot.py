...
# CONFIGURATION
# ============================================================================
# Default values (will be overridden by config.py/.env). Keeping these here
...
try:
    ...
    VARIANT = _CFG.VARIANT or VARIANT
    CONFIG_OFFICIAL_FLAG = _CFG.CONFIG_OFFICIAL_FLAG or CONFIG_OFFICIAL_FLAG
    ROM_TYPE = _CFG.ROM_TYPE or ROM_TYPE
    CONFIG_CHATID = _CFG.CONFIG_CHATID or CONFIG_CHATID
    CONFIG_BOT_TOKEN = _CFG.CONFIG_BOT_TOKEN or CONFIG_BOT_TOKEN
    ...
    PIN_SUCCESS_MESSAGE = bool(getattr(_CFG, 'PIN_SUCCESS_MESSAGE', PIN_SUCCESS_MESSAGE))
except Exception as _e:
    _CFG = None  # config not available; continue with defaults
...
def main():
    ...
    # Reload config if --env-file provided
    if args.env_file:
        try:
            from config import BotConfig as _BotConfig
            cfg = _BotConfig.load(args.env_file)
        except Exception:
            cfg = _CFG
    else:
        cfg = _CFG

    print("🚀 CI Bot - Starting process...")

    # Prepare host and sync sources if configured
    def _run(cmd_list, env=None):
        print(f"$ {' '.join(cmd_list)}")
        return subprocess.run(cmd_list, env=env, check=True)

    # Ensure scripts are executable if present
    for script in ('setup_build_env.sh', 'setup_xaga_env.sh'):
    ...
    if cfg is not None:
        ...
        if getattr(cfg, 'RUN_SOURCE_SYNC', False):
            ...
            try:
                if getattr(cfg, 'DRY_RUN', False):
                    env_preview = ' '.join([f"{k}={v}" for k, v in cfg.to_script_env().items()])
                    print(f"[DRY_RUN] Would run {env_preview} ./setup_xaga_env.sh")
                else:
                    _run(['./setup_xaga_env.sh'], env=env)
            except FileNotFoundError:
                print("⚠️  setup_xaga_env.sh not found; skipping.")

        # If we only want to prepare and not build
        if args.prepare_only or not getattr(cfg, 'RUN_BUILD', False):
            print("✅ Preparation step finished. Exiting (RUN_BUILD=false or --prepare-only).")
            return

        # If we will build, change into WORKDIR where sources were synced
        if cfg.WORKDIR and os.path.isdir(cfg.WORKDIR):
            os.chdir(cfg.WORKDIR)
            ROOT_DIRECTORY = os.getcwd()
            print(f"📁 Changed working directory to: {ROOT_DIRECTORY}")

        # Update globals from config (may reflect after chdir)
        TELEGRAM_BASE_URL = f"https://api.telegram.org/bot{cfg.CONFIG_BOT_TOKEN or CONFIG_BOT_TOKEN}"
        # Push config overrides into globals used by build and notifications
        globals().update({
            'DEVICE': cfg.DEVICE or DEVICE,
            'VARIANT': cfg.VARIANT or VARIANT,
            'CONFIG_OFFICIAL_FLAG': cfg.CONFIG_OFFICIAL_FLAG or CONFIG_OFFICIAL_FLAG,
            'ROM_TYPE': cfg.ROM_TYPE or ROM_TYPE,
            'CONFIG_CHATID': cfg.CONFIG_CHATID or CONFIG_CHATID,
            'CONFIG_BOT_TOKEN': cfg.CONFIG_BOT_TOKEN or CONFIG_BOT_TOKEN,
